...
export class MultiplayerTournament {
  ...
  init() {
    ...
    this.client.on('waitingRoomUpdate', (payload) => this.updateWaitingRoom(payload));
    this.client.on('startHeroSelection', () => this.startHeroSelection());
  ...
  updateWaitingRoom(payload) {
    const counterEl = this.container.querySelector('#player-counter');
    const listEl = this.container.querySelector('#waiting-players-list');
    const statusEl = this.container.querySelector('#player-count');
    
    if (counterEl) {
      counterEl.textContent = `${payload.playerCount}/8`;
    }
